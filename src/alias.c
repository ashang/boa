 ...
 *  Some changes Copyright (C) 1996, 2000 Larry Doolittle <ldoolitt@boa.org>
 *  Some changes Copyright (C) 1996 Jon Nelson <jnelson@boa.org>
...
#define BS MAX_HEADER_LENGTH
	...
	if(!strncmp(req->request_uri, current->fakename, 
	  current->fake_len)) {
		...
		snprintf(buffer, BS, "%s%s", current->realname,
	...
	snprintf(buffer, BS, "%s/%s%s", user_homedir, user_dir, req_urip);
    }
    else {
	...
	snprintf(buffer, BS, "%s/%d.%d.%d.%d%s", document_root,
	    iplocal[0], iplocal[1], iplocal[2], iplocal[3], req->request_uri);
#else
        snprintf(buffer, BS, "%s%s", document_root, req->request_uri);
#endif
    }
...
int init_script_alias(request * req, alias * current)
{
    ...
    snprintf(path_translated, BS, "%s%s", current->realname, 
      &req->request_uri[current->fake_len]);
